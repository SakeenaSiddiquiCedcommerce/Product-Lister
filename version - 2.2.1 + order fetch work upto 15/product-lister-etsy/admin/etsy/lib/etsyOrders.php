...
class Ced_Order_Get {
	...
	public function get_orders( $shopId, $is_sync = false ) {
        ...
        $result = file_get_contents( CED_ETSY_DIRPATH . 'admin/etsy/lib/json/woo-etsy-test-order.json' ); //testing
        $result = json_decode($result, true);
        $results = array_chunk($result['results'], 10);
		if ( isset( $results[0] ) && ! empty( $results[0] ) ) {
			$this->createLocalOrder( $results[0], $shopId );
		}

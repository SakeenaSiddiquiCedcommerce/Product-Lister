...
class Woocommmerce_Etsy_Integration_Admin {
	...
	*/
		...
		if ( ! empty( $shops ) ) {
			foreach ( $shops as $key => $value ) {
				...
				if ( isset( $value['details']['ced_shop_account_status'] ) && 'Active' == $value['details']['ced_shop_account_status'] ) {
					$global_setting_page_url = admin_url() . 'admin.php?page=ced_etsy_lister&section=ced-etsy-settings&shop_name=' . $key;
					$category_page_url       = admin_url() . 'admin.php?page=ced_etsy_lister&section=category-mapping-view&shop_name=' . $key;
					$profile_page_url        = admin_url() . 'admin.php?page=ced_etsy_lister&section=profiles-view&shop_name=' . $key;
					$products_page_url       = admin_url() . 'admin.php?page=ced_etsy_lister&section=products-view&shop_name=' . $key;
					$timeline_page_url       = admin_url() . 'admin.php?page=ced_etsy_lister&section=etsy-timeline&shop_name=' . $key;
					$admin_bar->add_menu(
						array(
							'parent' => 'product-lister-etsy',
							'title'  => __( 'Global Settings' ),
							'id'     => 'product-lister-global',
							'href'   => $global_setting_page_url,
						)
					);
					$admin_bar->add_menu(
						array(
							...
							'id'     => 'product-lister-category',
							'href'   => $category_page_url,
						)
					);
					$admin_bar->add_menu(
						array(
					...
					$admin_bar->add_menu(
						array(
							'parent' => 'product-lister-etsy',
							'title'  => __( 'Timeline' ),
							'id'     => 'product-lister-timeline',
							'href'   => $timeline_page_url,
						)
					);
				}
			}
	...
	/**
	 * **************************************************************
	 * Woocommerce_Etsy_Integration_Admin adding delete functionality on timeline
	 * **************************************************************
	 */

	public function ced_etsy_adding_delete_functionality_on_timeline() {
		$sanitized_array = filter_input_array( INPUT_POST, FILTER_SANITIZE_FULL_SPECIAL_CHARS );
		$option_name     = isset( $sanitized_array['option_name'] ) ? $sanitized_array['option_name'] : '';
		$update_logs     = update_option( $option_name, '' );
		if ( $update_logs ) {
			echo json_encode( 'Option Updated Successfully' );
			wp_die();
	...
	public function ced_etsy_add_searching_on_timeline() {
		...
		foreach ( $decoded_info as $key  => $info ) {
			...
			if ( str_contains( $sku_to_search, $seach_sku_ipt ) || ( isset( $info['post_title'] ) && str_contains( strtolower( $info['post_title'] ), $seach_sku_ipt ) ) || ( isset( $order_number ) && str_contains( (string) $order_number, (string) $seach_sku_ipt ) ) ) {
				$html .= '<tr class="ced_etsy_log_rows">
   						  <td>
   							<span class="log_item_label log_details" id="abc">
   								<a>' . $info['post_title'] . '</a>
   							</span>
   							<span class="log_message show" style="display: none;">
   								<h3>Input payload for ' . $info['post_title'] . '</h3>
   								<button id="ced_close_log_message" style="left: 66%;">Close</button>
   								<pre>' . json_encode( $info['input_payload'], JSON_PRETTY_PRINT ) . '</pre>
   							</span>
   						</td>
   						<td>
   						...
   						<td>
   							<span class="">' . ( ( $info['is_auto'] ) ? 'Automatic' : 'Manual' ) . '</span>
   						</td>
   						<td>
   							<span class="etsy_log_success log_details">' . ( ( isset( $info['response'] ) && ! empty( $info['response'] ) ) ? 'Success' : 'failed' ) . '</span>
   							<span class="log_message show" style="display: none;">
   								<h3>Response payload for ' . $info['post_title'] . '</h3>
   								<button id="ced_close_log_message" style="left: 66%;">Close</button>
   								<pre>' . json_encode( $info['response'], JSON_PRETTY_PRINT ) . '</pre>
   							</span>
   						</td>
   					</tr>';
			}

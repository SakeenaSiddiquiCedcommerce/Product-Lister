...
if ( ! class_exists( 'Ced_Product_Upload' ) ) {
	class Ced_Product_Upload extends Etsy_Request {
		...
		public function do_image_upload( $l_id, $pr_id, $img_id, $shop_name ) {
			...
			$image_custom_path = $upload_dir['basedir'] . '/EtsyWooIntegration/' . $image_name;
			if ( ! is_dir( ( $upload_dir['basedir'] . '/EtsyWooIntegration/' ) ) ) {
				mkdir( ( $upload_dir['basedir'] . '/EtsyWooIntegration/' ), 0755 );
			}
			if ( exif_imagetype( $image_path ) == IMAGETYPE_WEBP && ! file_exists( $image_custom_path ) ) {
				$im = imagecreatefromwebp( $image_path );
				imagejpeg( $im, $upload_dir['basedir'] . '/EtsyWooIntegration/' . $image_name, 100 );
				imagedestroy( $im );

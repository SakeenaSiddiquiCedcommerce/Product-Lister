...
class Ced_Etsy_Account_Table extends WP_List_Table {
	...
	public function column_configure( $item ) {
		...
		$configure = '<a class="button-primary" href="' . admin_url( 'admin.php?page=ced_etsy_lister&section=ced-etsy-settings&shop_name=' . $item['details']['ced_etsy_shop_name'] . '' ) . '">' . __( 'Configure', 'woocommerce-etsy-integration' ) . '</a>';
		return $configure;
	...
	public function renderHTML() {
		...
		<div class="ced_etsy_wrap ced_etsy_wrap_extn">
			...
			<div class="ced_etsy_setting_header cedcommerce-top-border">
				...
				$count = self::get_count();
				if ( $count < 1 ) {
					$message = 'To start syncing your products, begin with connecting your Etsy account with the plugin using the Connect Etsy Account option on top right.';
					...
					$message = 'Start syncing your products from your WooCommerce store to Etsy. Click configure button to explore further .';
				}
				?>
			</div>
			<div class="ced_etsy_body">
				<div class="ced_etsy_welcome_notice"><h3>Welcome to Product Lister For Etsy ! </h3>
				<?php
	...
	public function process_bulk_action() {
		...
		if ( 'bulk-delete' === $this->current_action() || ( isset( $_GET['action'] ) && 'bulk-delete' === $_GET['action'] ) || isset( $_GET['action2'] ) && 'bulk-delete' === $_GET['action2'] ) {
			...
			$redirectURL = get_admin_url() . 'admin.php?page=ced_etsy_lister';
			wp_redirect( $redirectURL );
		} elseif ( isset( $_GET['section'] ) ) {
			echo "<div class='ced_etsy_body'>";
			if ( file_exists( CED_ETSY_DIRPATH . 'admin/partials/' . $this->current_action() . '.php' ) ) {
...
if ( isset( $_POST['ced_etsy_authorise_account_button'] ) && 'Authorize' == $_POST['ced_etsy_authorise_account_button'] ) {
	...
	$verifier       = base64_encode( admin_url( 'admin.php?page=ced_etsy_lister' ) );

(function( $ ) {
	...
	$( document ).on(
		...
		function(){
			...
			$( this ).next( '.ced_etsy_child_element' ).slideToggle( 'slow' );
		}
	);
		...
		$( document ).on(
			...
			function(e){
				...
				if (operation <= 0 ) {
					...
					performBulkAction( etsy_products_id, operation, total_products );
				}

			}
		);

	function performBulkAction(etsy_products_id, operation, total_products )
		 {
		if (etsy_products_id == "") {
				var notice = "";
			 ...
			 $( '.ced_etsy_loader' ).show();
		$( '.ced_progress' ).show();
			 var etsy_products_id_to_perform = etsy_products_id[0];var total_processed             = 0;
			...
			$.ajax(
					...
					success: function(response)
				{
						...
						if (response1 == "Shop is Not Active") {
							...
							var notice           = "";
							notice              += "<div class='notice notice-success'>" + response.message + "</div>";
							$( ".success-admin-notices" ).append( notice );
							if (Response_message == 'Product ' + id + ' Deleted Successfully') {
								$( "#" + id + "" ).html( '<b class="not_completed">Not on Etsy</b>' );
								$( "." + id + "" ).remove();
							} else {
								$( "#" + id + "" ).html( '<span class="success_upload_on_etsy">Present on Etsy</span>' );
							}

							if (remainig_products_id == "") {
								$( '.ced_etsy_loader' ).hide();
								$( '.ced_progress' ).hide();
								$( "#ced_progress" ).attr( "max", '' );
								$( "#ced_progress" ).attr( "value", 0 );
								return;
							} else {
								performBulkAction( remainig_products_id, operation, total_products );
							}

						} else if (response.status == 400) {
							$( "#ced_progress" ).attr( "value", total_processed );
							var notice = "";
							notice    += "<div class='notice notice-error'>" + response.message + "</div>";
							$( ".success-admin-notices" ).append( notice );
							if (remainig_products_id == "") {
								$( '.ced_etsy_loader' ).hide();
								$( '.ced_progress' ).hide();
		...
		/**
		 * FUNCTION TO ADD CLEAR LOGS FUNCTIONALITY ON TIME LINE FOR INVENTORY
		 *
		 * @param  shop_name Active Shop Name Of Etsy
		 * @param  option_name  us exactly option_key to get values from option table
		 */

		$( document ).on(
			'click',
			'#ced_clear_log_Inventory',
			function(){
				   var conf = confirm( 'Are you sure want to delete all this logs?' );
				if (conf) {
					$.ajax(
						{
							url : ajaxUrl,
							data : {
								ajax_nonce   : ajaxNonce,
								action       : 'ced_etsy_adding_delete_functionality_on_timeline',
								option_name  : 'ced_etsy_product_inventory_logs_' + shop_name
							},
							type : 'POST',
							dataType : 'json',
							success : function( response ){
		...
		/**
		 * FUNCTION TO ADD CLEAR LOGS FUNCTIONALITY ON TIME LINE FOR PRODUCTS
		 *
		 * @param  shop_name Active Shop Name Of Etsy
		 * @param  option_name  us exactly option_key to get values from option table
		 */
					...
					$.ajax(
						{
							...
							data : {
								ajax_nonce   : ajaxNonce,
								action       : 'ced_etsy_adding_delete_functionality_on_timeline',
								option_name  : 'ced_etsy_product_logs_' + shop_name
							},
							type : 'POST',
							dataType : 'json',
							success : function( response ){
		...
		/**
		 * FUNCTION TO FOR AJAX CALL FOR SEARCHING ON TIMELINE FOR INVENTORY
		 *
		 * @param  search_key for searching the inventory
		 * @param  shop_name Active Shop Name Of Etsy
		 * @param  option_name  us exactly option_key to get values from option table
		 */
		...
		$( document ).on(
			'keyup',
			'#ced_etsy_search_Inventory',
			function(){
				var search_key = $( '#ced_etsy_search_Inventory' ).val();
				$.ajax(
					{
						url : ajaxUrl,
						data : {
							ajax_nonce   : ajaxNonce,
							action       : 'ced_etsy_add_searching_on_timeline',
						...
						dataType : 'json',
						success : function( response ){
							$( '.ced_etsy_product_inventory_logs_' + shop_name ).html( response );
						}
					}
				);
		...
		/**
		 * FUNCTION TO FOR AJAX CALL FOR SEARCHING ON TIMELINE FOR PRODUCT
		 *
		 * @param  search_key for searching the inventory
		 * @param  shop_name Active Shop Name Of Etsy
		 * @param  option_name  us exactly option_key to get values from option table
		...
		$( document ).on(
			'keyup',
			'#ced_etsy_search_Product',
			function(){
				 var search_key = $( '#ced_etsy_search_Product' ).val();
				$.ajax(
					{
						url : ajaxUrl,
						data : {
						...
						dataType : 'json',
						success : function( response ){
							$( '.ced_etsy_product_logs_' + shop_name ).html( response );
						}
					}
				);

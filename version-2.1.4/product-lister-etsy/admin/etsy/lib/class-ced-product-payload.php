...
trait Ced_Product_Payload {
	...
	public function get_formatted_data( $product_id = '', $shop_name = '' ) {
		...
		$this->ced_global_settings = $this->ced_global_settings = get_option( 'ced_etsy_global_settings', array() );
		...
		$product_field_instance = new Ced_Etsy_Product_Fields();
		...
		foreach ( $sections as $section ) {
			foreach ( $etsy_data_field[ $section ] as $section_attributes ) {
				...
				if ( '' == $pro_val ) {
					$pro_val = isset( $this->ced_global_settings[$shop_name]['product_data'][ $meta_key ]['default'] ) ? $this->ced_global_settings[$shop_name]['product_data'][ $meta_key ]['default'] : '';// getting info from global level
				}
		...
		foreach ( $required_fields as $index ) {
			if ( method_exists( $this, 'get_' . $index ) ) {
				...
				if ( false !== $info ) {
					...
					$valid      = false;
					$error_msg .= '[ ' . ucwords( str_replace('_', ' ', $index ) ) . ' is required but missing from product information ]';
				}
			}
		}
	...
	public function get_state() {
		$product_list_type = ! empty( $this->pro_data['product_list_type'] ) ? $this->pro_data['product_list_type'] : 'draft';

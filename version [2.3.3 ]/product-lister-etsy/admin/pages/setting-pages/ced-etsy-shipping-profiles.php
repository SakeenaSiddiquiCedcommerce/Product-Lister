...
$parameters['body'] = json_encode(
	array(
		'action'       => 'get_shipping_profiles',
		'shop_name'    => $activeShop,
		'shop_id'      => $shop_id,
		'user_details' => $shopDetails,
	)
);
...
$shopShippingTemplates = etsy_request()->ced_remote_request( $activeShop, $_action, 'GET' );
		...
		<div id="update-button">
				<table class="wp-list-table widefat ced_etsy_config_table"  id="">
					...
					?>
							<td>
								<?php
								$url = admin_url( 'admin.php?page=ced_etsy_lister&section=add-shipping-profile-view&shop_name=' . $activeShop );
								?>
								<a href="<?php echo esc_attr( $url ); ?>" class="button-primary " >Create New</a>
							</td>
						</tr>
					</tbody>
				</table>
